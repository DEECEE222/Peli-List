...
export function Recommendations({ movies, onAdd }: RecommendationsProps) {
  ...
  // Find favorite genre from user's movies
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Search TMDB for popular movies of that genre
    ...
    fetch(
      ...
      .then(r => r.json())
      .then(data => {
        // Filter out movies already in user's list
    ...
    <section className="mt-12">
      ...
      {!loading && recs.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {recs.map((movie, i) => (
            <div key={movie.id}
              className="group relative bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-purple-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/10 hover:-translate-y-1"
